...
class ValenceQAgent:
    ...
    def get_probs(self, obs: Tuple[int, int, bool], avail_actions: List = None):
        ...
        ----------
        obs : Tuple[int, int, bool]
            The current state observation representedz as a tuple.
        avail_actions : list, optional
            A list of available actions. If None, all actions are assumed available, by default None.
...
for idx, task in tqdm(enumerate(agent_data['task']), total=len(agent_data['task']), desc="Inferring task data."):
    ...
    for recov_agent in recov_agent_names:
        ...
        recovery_data['recov_agent_model'].append(recov_agent.split("_")[1])
        recovery_data['recov_agent_valence'].append(recov_agent.split("_")[0])

```

```python
task_recov = recov_data.query("task=='risk-sensitive' and orig_agent_valence in ['pessimistic', 'neutral', 'optimistic'] and recov_agent_valence == 'valence'")
original_learning_rate_pos = [task_recov.iloc[i, :]["orig_params"][1] for i in range(task_recov.shape[0])]
original_learning_rate_neg = [task_recov.iloc[i, :]["orig_params"][0] for i in range(task_recov.shape[0])]


fig, axes = plt.subplots(1, 3, figsize=(15, 5))

sns.regplot(x=original_learning_rate_neg, y=task_recov.learning_rate_mf_neg, ax=axes[0])
sns.regplot(x=original_learning_rate_pos, y=task_recov.learning_rate_mf_pos, ax=axes[0])

task_recov = recov_data.query("task=='gonogo' and orig_agent_valence in ['pessimistic', 'neutral', 'optimistic'] and recov_agent_valence == 'valence'")
original_learning_rate_pos = [task_recov.iloc[i, :]["orig_params"][1] for i in range(task_recov.shape[0])]
original_learning_rate_neg = [task_recov.iloc[i, :]["orig_params"][0] for i in range(task_recov.shape[0])]

sns.regplot(x=original_learning_rate_neg, y=task_recov.learning_rate_mf_neg, ax=axes[1])
sns.regplot(x=original_learning_rate_pos, y=task_recov.learning_rate_mf_pos, ax=axes[1])

task_recov = recov_data.query("task=='two-step' and orig_agent_valence in ['pessimistic', 'neutral', 'optimistic'] and recov_agent_valence == 'valence'")
original_learning_rate_pos = [task_recov.iloc[i, :]["orig_params"][1] for i in range(task_recov.shape[0])]
original_learning_rate_neg = [task_recov.iloc[i, :]["orig_params"][0] for i in range(task_recov.shape[0])]

sns.regplot(x=original_learning_rate_neg, y=task_recov.learning_rate_mf_neg, ax=axes[2])
sns.regplot(x=original_learning_rate_pos, y=task_recov.learning_rate_mf_pos, ax=axes[2])
...
sns.heatmap(calc_conf_matrix("risk-sensitive", "orig_agent_valence", "recov_agent_valence"), annot=True, cmap="viridis", cbar=False, ax=axes[0])
sns.heatmap(calc_conf_matrix("gonogo", "orig_agent_valence", "recov_agent_valence"), annot=True, cmap="viridis", cbar=False, ax=axes[1])
sns.heatmap(calc_conf_matrix("two-step", "orig_agent_model", "recov_agent_model"), annot=True, cmap="viridis", cbar=False, ax=axes[2])
...
for task in tasks:
        for n in range(2):
            env = get_env(task,'psychopy-simulate')
            ...
            for ag, agn, par in zip(agents, agent_names, params):
                    ...
                    df, _, _ = run_task(env, logger=simlog, agent=ag)
                    agent_data['task'].append(task)
                    agent_data['agent'].append(agn)
                    agent_data['agent_model'].append(agn.split('_')[1] if agn != "random" else "random")
                    agent_data['agent_valence'].append(agn.split('_')[0] if agn != "random" else "random")
...
```python
summary_dfs = {ii : [] for ii in ['risk-sensitive', 'gonogo', 'two-step']}
...
for df, task, ag, agm, agv in zip(agent_data['dataframes'], agent_data['task'], agent_data['agent'], agent_data['agent_model'], agent_data['agent_valence']):
    ...
    if task == 'risk-sensitive': 
        ...
        df.loc[df.action=='n/a', 'action'] = np.nan
        df.loc[:, 'action'] = df.action.astype(float)
        sum_df = summary_df_gonogo(df, ag)
...
```python
sns.barplot(rs_df_cc.query("metric == 'proportion' and trial_classification=='risky'"), x='agent_valence', y='value')
```

```python
plt.figure(figsize=(15, 5))
sns.barplot(rs_df_cc.query("metric == 'proportion' and trial_classification=='test'"), x='participant', y='value')
```

```python
sns.catplot(ts_df_cc.query("metric == 'proportion'"), x='trial_type', y='value', col='agent_model', kind='bar', col_wrap=2)
```
